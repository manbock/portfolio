...
function canvas(){
  ...
  // canvas.getContext("2d"); ➡ canvas 사용의 필수❗❗❗
...
function scaleImage(img,ctx){
  ...
  // let ratio=Math.max(10, 20)//더 큰값을 찾음 = 20
  let ratio=Math.max(hRatio, vRatio)
  let centershift_x=(canvas.width - img.width*ratio)/2;
  let centershift_y=(canvas.height - img.height*ratio)/2;
    
  ctx.clearRect(0,0,canvas.width,canvas.height)//화면 지우기
  //canvas에 이미지를 표현할때 drawImage(image, x, y, width, height)
  //sx, sy, swidth --> 소스자체, dx,dy,dwidth,dheight-->소스만들어진후 설정
...
ScrollTrigger.create({
  ...
  pin:true,//스크롤하는동안 트리고 요소가 화면고정됨
  start:"top top",
  end:"1000%  top",
})
